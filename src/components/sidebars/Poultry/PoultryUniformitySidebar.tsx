...
export type PoultryUniformityTab = 
  | 'uniformity-calculator' 
  | 'formula-guide';

interface UniformitySidebarProps {
  isOpen: boolean;
  onClose: () => void;
  onNavigate?: (id: PoultryUniformityTab, label: string) => void;
  activeTab?: PoultryUniformityTab;
  setActiveTab?: (tab: PoultryUniformityTab) => void;
  userEmail?: string;
}

export default function PoultryUniformitySidebar({
  isOpen,
  onClose,
  onNavigate,
  activeTab = 'uniformity-calculator',
  setActiveTab,
  userEmail
      ...
      <div className="relative w-70 max-w-[80vw] bg-white h-full shadow-2xl z-50 flex flex-col justify-between overflow-y-auto">
        <div>
          ...
          <div className="bg-amber-600 text-white p-4 flex justify-between items-center sticky top-0 z-10">
            <div>
              <h2 className="font-bold text-base sm:text-lg leading-tight">Uniformity Manager</h2>
              <p className="text-[11px] text-amber-100">Layer Flock Management & CV%</p>
            </div>
            <button 
              onClick={onClose} 
          ...
          <nav className="p-3 space-y-1">
            {/* 1. Uniformity Calculator */}
            <button
              onClick={() => handleTabClick('uniformity-calculator', 'Uniformity Calculator')}
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <span>Uniformity Calculator</span>
              </div>
